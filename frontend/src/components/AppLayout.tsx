import { BarChart3, BriefcaseBusiness, CreditCard, LayoutDashboard, LogOut, Moon, PiggyBank, Settings, Sun, Upload } from "lucide-react";
import { NavLink, Outlet } from "react-router-dom";
import { useAuthStore } from "../store/auth";
import { useThemeStore } from "../store/theme";
import { CopilotWidget } from "./CopilotWidget";

// Copilot is deliberately absent — it lives in the floating widget, reachable
// from every page rather than being a destination of its own.
const navItems = [
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(13,148,136,0.12),transparent_28%),radial-gradient(circle_at_top_right,rgba(37,99,235,0.10),transparent_22%),linear-gradient(180deg,#f8fafc_0%,#eef2ff_100%)] text-slate-950 dark:bg-[radial-gradient(circle_at_top_left,rgba(13,148,136,0.15),transparent_28%),radial-gradient(circle_at_top_right,rgba(37,99,235,0.12),transparent_22%),linear-gradient(180deg,#020617_0%,#0f172a_100%)] dark:text-slate-50">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-200/70 bg-white/80 p-4 backdrop-blur-xl xl:block dark:border-slate-800 dark:bg-slate-950/80">
        <div className="mb-8 flex items-center gap-3 rounded-2xl border border-slate-200/60 bg-white/50 px-4 py-3.5 shadow-xs transition hover:border-slate-300 dark:border-slate-800/80 dark:bg-slate-900/40">
          ...
          <div>
            ...
            <p className="text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Personal AI CFO</p>
          </div>
        </div>
      ...
      <div className="xl:pl-72">
        <header className="sticky top-0 z-20 border-b border-slate-200/60 bg-white/60 px-6 py-4 backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-950/60">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Welcome back</p>
              <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white mt-0.5">{user?.full_name ?? ""}</h1>
            </div>
