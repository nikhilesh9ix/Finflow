...
def _tx(
    ...
    *,
    ...
    return mongo.insert(db, mongo.TRANSACTIONS, {
        "user_id": user.id,
        "transaction_date": d,
...
def _budget(db: Database, user: User, category: str, limit: Decimal) -> dict:
    return mongo.insert(db, mongo.BUDGETS, {
        "user_id": user.id, "category": category, "monthly_limit": limit, "priority": "normal",
    })
...
class TestCurrentMonthSummary:
    def test_empty_user_returns_zeros(self, db: Database, user: User) -> None:
    ...
    def test_income_and_expense_separated(self, db: Database, user: User) -> None:
    ...
    def test_transfers_excluded(self, db: Database, user: User) -> None:
        _tx(db, user, amount=Decimal("5000"), tx_type="transfer", category="Transfer")
        result = analytics.current_month_summary(db, user.id, "2026-07")
        assert result["income"] == _zero()
        assert result["expense"] == _zero()

    def test_other_month_excluded(self, db: Database, user: User) -> None:
        _tx(db, user, amount=Decimal("-2000"), tx_type="expense", category="Food", d=date(2026, 6, 15))
        result = analytics.current_month_summary(db, user.id, "2026-07")
        assert result["expense"] == _zero()

    def test_multiple_expenses_summed(self, db: Database, user: User) -> None:
...
class TestCategoryBreakdown:
    def test_groups_by_category(self, db: Database, user: User) -> None:
        _tx(db, user, amount=Decimal("-500"), tx_type="expense", category="Food")
    ...
    def test_income_excluded_from_breakdown(self, db: Database, user: User) -> None:
        _tx(db, user, amount=Decimal("50000"), tx_type="income", category="Salary/Income")
        result = analytics.category_breakdown(db, user.id, "2026-07")
        assert not any(r["category"] == "Salary/Income" for r in result)

    def test_sorted_descending_by_amount(self, db: Database, user: User) -> None:
...
class TestBudgetAlerts:
    def test_no_spend_shows_zero_and_safe(self, db: Database, user: User) -> None:
    ...
    def test_overspent_status(self, db: Database, user: User) -> None:
    ...
    def test_warning_at_80_percent(self, db: Database, user: User) -> None:
    ...
    def test_usage_percent_calculated(self, db: Database, user: User) -> None:
    ...
    def test_spend_from_different_category_not_counted(self, db: Database, user: User) -> None:
        _budget(db, user, "Food", Decimal("3000"))
        _tx(db, user, amount=Decimal("-5000"), tx_type="expense", category="Rent")
        alerts = analytics.budget_alerts(db, user.id, "2026-07")
        assert alerts[0]["spent"] == _zero()

    def test_other_month_spend_not_counted(self, db: Database, user: User) -> None:
        _budget(db, user, "Food", Decimal("3000"))
...
class TestRecurringTransactions:
    def test_merchant_appearing_twice_detected(self, db: Database, user: User) -> None:
    ...
    def test_single_transaction_not_recurring(self, db: Database, user: User) -> None:
        _tx(db, user, amount=Decimal("-500"), tx_type="expense", category="Food",
            merchant="RandomShop")
        result = analytics.recurring_transactions(db, user.id)
        assert not any(r["merchant"] == "RandomShop" for r in result)

    def test_average_amount_computed(self, db: Database, user: User) -> None:
...
class TestIncomeVsExpense:
    def test_returns_both_sides_per_month(self, db: Database, user: User) -> None:
    ...
    def test_multiple_months_ordered(self, db: Database, user: User) -> None:
